...
// #define	GENTITYNUM_BITS		11		// don't need to send any more		(SA) upped 4/21/2001 adjusted: tr_local.h
// (802-822), tr_main.c (1501), sv_snapshot (206)
...
typedef struct
{ // A structure representing a player's scoreboard
	int score;
	int deaths;
	int kills;
	int assists;
...
typedef struct playerState_s
{
	...
	// Animations as in mp/playeranim.script and animtrees/multiplayer.atr, it also depends on mp/playeranimtypes.txt
...
struct gentity_s
{
	...
	/*
			...
			int clipmask;                   // brushes with this content value will be collided against
																			// when moving.  items and corpses do not
	   collide against
...
struct gclient_s
{
	...
	/*	int timeResidual;
		...
		int PCSpecialPickedUpCount;         // JPW NERVE used to count # of times somebody's picked up this LTs ammo (or
	   medic health) (for scoring) int saved_persistant[MAX_PERSISTANT];           // DHM - Nerve :: Save ps->persistant
	   here during Limbo

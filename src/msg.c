...
netFieldList_t netFieldList[] = { { NETFE(entityStateFields) }, { NETFE(playerEntityStateFields) },
	...
	{ NETFE(fxStateFields) }, { NETFE(loopFxEntityStateFields) }, { NETFE(entityStateFields) },
	{ NETFE(entityStateFields) }, { NETFE(helicopterEntityStateFields) }, { NETFE(planeStateFields) },
	{ NETFE(vehicleEntityStateFields) }, { NETFE(entityStateFields) }, { NETFE(entityStateFields) },
...
*/
__regparm3 void MSG_WriteDeltaField(struct snapshotInfo_s *snapInfo, msg_t *msg, const int time, const byte *from,
	const byte *to, const struct netField_s *field, int fieldNum, byte forceSend)
{
	int nullfield;
	int32_t timetodata;
	int32_t absbits;
	...
	case 0:
		...
		MSG_WriteBit1(msg);
		if (int32todata == 0x80000000 || floattodata != (float)int32todatafromfloat || int32todatafromfloat + 4096 < 0
			|| int32todatafromfloat + 4096 > 0x1FFF || int32fromdatafromfloat + 4096 < 0
			|| int32fromdatafromfloat + 4096 > 0x1FFF)
		{
			MSG_WriteBit1(msg);
	...
	case -89:
		if (int32todata == 0x80000000 || floattodata != (float)int32todatafromfloat || int32todatafromfloat + 4096 < 0
			|| int32todatafromfloat + 4096 > 0x1FFF)
		{
			MSG_WriteBit1(msg);
			MSG_WriteLong(msg, uint32todata ^ uint32fromdata);
			break;
...
int MSG_WriteDeltaStruct(snapshotInfo_t *snapInfo, msg_t *msg, const int time, const byte *from, const byte *to,
	qboolean force, int numFields, int indexBits, netField_t *stateFields, qboolean bChangeBit)
{
	int i, lc;
	int *fromF, *toF;
...
int MSG_WriteEntityDelta(struct snapshotInfo_s *snapInfo, msg_t *msg, const int time, const byte *from, const byte *to,
	qboolean force, int numFields, int indexBits, netField_t *stateFields)
{
	...
	entityState_t *to, qboolean force)
{
	...
	MSG_WriteEntityDelta(snapInfo, msg, time, (const byte *)from, (const byte *)to, force, fieldtype->numFields, 10,
		fieldtype->field);
}
...
netField_t playerStateFields[] = { { PSF(commandTime), -97, 0 }, { PSF(viewangles[1]), -87, 0 },
	{ PSF(viewangles[0]), -87, 0 }, { PSF(viewangles[2]), -87, 0 }, { PSF(origin[0]), -88, 3 },
	{ PSF(origin[1]), -88, 3 }, { PSF(bobCycle), 8, 3 }, { PSF(velocity[1]), -88, 3 }, { PSF(velocity[0]), -88, 3 },
	{ PSF(movementDir), -8, 3 }, { PSF(eventSequence), 8, 0 }, { PSF(legsAnim), 10, 0 }, { PSF(origin[2]), -88, 3 },
	{ PSF(weaponTime), -16, 0 }, { PSF(aimSpreadScale), -88, 0 }, { PSF(torsoTimer), 16, 0 }, { PSF(pm_flags), 21, 0 },
	{ PSF(weapAnim), 10, 0 }, { PSF(weaponstate), 5, 0 }, { PSF(velocity[2]), -88, 3 }, { PSF(events[0]), 8, 0 },
	{ PSF(events[1]), 8, 0 }, { PSF(events[2]), 8, 0 }, { PSF(events[3]), 8, 0 }, { PSF(eventParms[0]), 8, 0 },
	{ PSF(eventParms[1]), 8, 0 }, { PSF(eventParms[2]), 8, 0 }, { PSF(eventParms[3]), 8, 0 }, { PSF(torsoAnim), 10, 0 },
	{ PSF(holdBreathScale), -88, 0 }, { PSF(eFlags), -98, 0 }, { PSF(viewHeightCurrent), -88, 0 },
	{ PSF(fWeaponPosFrac), -88, 0 }, { PSF(legsTimer), 16, 0 }, { PSF(viewHeightTarget), -8, 0 },
	{ PSF(sprintState.lastSprintStart), -97, 0 }, { PSF(sprintState.lastSprintEnd), -97, 0 }, { PSF(weapon), 7, 0 },
	{ PSF(weaponDelay), -16, 0 }, { PSF(sprintState.sprintStartMaxLength), 14, 0 }, { PSF(weapFlags), 9, 0 },
	{ PSF(groundEntityNum), 10, 0 }, { PSF(damageTimer), 10, 0 }, { PSF(weapons[0]), 32, 0 },
	{ PSF(weapons[1]), 32, 0 }, { PSF(weaponold[0]), 32, 0 }, { PSF(delta_angles[1]), -100, 0 },
	{ PSF(offHandIndex), 7, 0 }, { PSF(pm_time), -16, 0 }, { PSF(otherFlags), 5, 0 },
	{ PSF(moveSpeedScaleMultiplier), 0, 0 }, { PSF(perks), 32, 0 }, { PSF(killCamEntity), 10, 0 },
	{ PSF(throwBackGrenadeOwner), 10, 0 }, { PSF(actionSlotType[2]), 2, 0 }, { PSF(delta_angles[0]), -100, 0 },
	{ PSF(speed), 16, 0 }, { PSF(viewlocked_entNum), 16, 0 }, { PSF(gravity), 16, 0 }, { PSF(actionSlotType[0]), 2, 0 },
	{ PSF(dofNearBlur), 0, 0 }, { PSF(dofFarBlur), 0, 0 }, { PSF(clientNum), 8, 0 }, { PSF(damageEvent), 8, 0 },
	{ PSF(viewHeightLerpTarget), -8, 0 }, { PSF(damageYaw), 8, 0 }, { PSF(viewmodelIndex), 9, 0 },
	{ PSF(damageDuration), 16, 0 }, { PSF(damagePitch), 8, 0 }, { PSF(flinchYawAnim), 2, 0 },
	{ PSF(weaponShotCount), 3, 0 }, { PSF(viewHeightLerpDown), 1, 2 }, { PSF(cursorHint), 8, 0 },
	{ PSF(cursorHintString), -8, 0 }, { PSF(cursorHintEntIndex), 10, 0 }, { PSF(viewHeightLerpTime), 32, 0 },
	{ PSF(offhandSecondary), 1, 2 }, { PSF(radarEnabled), 1, 2 }, { PSF(pm_type), 8, 0 }, { PSF(fTorsoPitch), 0, 0 },
	{ PSF(holdBreathTimer), 16, 0 }, { PSF(actionSlotParam[2]), 7, 0 }, { PSF(jumpTime), 32, 0 },
	{ PSF(mantleState.flags), 5, 0 }, { PSF(fWaistPitch), 0, 0 }, { PSF(grenadeTimeLeft), -16, 0 },
	{ PSF(proneDirection), 0, 0 }, { PSF(mantleState.timer), 32, 0 }, { PSF(damageCount), 7, 0 },
	{ PSF(shellshockTime), -97, 0 }, { PSF(shellshockDuration), 16, 2 },
	{ PSF(sprintState.sprintButtonUpRequired), 1, 2 }, { PSF(shellshockIndex), 4, 0 }, { PSF(proneTorsoPitch), 0, 0 },
	{ PSF(sprintState.sprintDelay), 1, 2 }, { PSF(actionSlotParam[3]), 7, 0 }, { PSF(weapons[3]), 32, 0 },
	{ PSF(actionSlotType[3]), 2, 0 }, { PSF(proneDirectionPitch), 0, 0 }, { PSF(jumpOriginZ), 0, 0 },
	{ PSF(mantleState.yaw), 0, 0 }, { PSF(mantleState.transIndex), 4, 0 }, { PSF(weaponrechamber[0]), 32, 0 },
	{ PSF(throwBackGrenadeTimeLeft), -16, 0 }, { PSF(weaponold[3]), 32, 0 }, { PSF(weaponold[1]), 32, 0 },
	{ PSF(foliageSoundTime), -97, 0 }, { PSF(vLadderVec[0]), 0, 0 }, { PSF(viewlocked), 2, 0 },
	{ PSF(deltaTime), 32, 0 }, { PSF(viewAngleClampRange[1]), 0, 0 }, { PSF(viewAngleClampBase[1]), 0, 0 },
	{ PSF(viewAngleClampRange[0]), 0, 0 }, { PSF(vLadderVec[1]), 0, 0 }, { PSF(locationSelectionInfo), 8, 0 },
	{ PSF(meleeChargeTime), -97, 0 }, { PSF(meleeChargeYaw), -100, 0 }, { PSF(meleeChargeDist), 8, 0 },
	{ PSF(iCompassPlayerInfo), 32, 0 }, { PSF(weapons[2]), 32, 0 }, { PSF(actionSlotType[1]), 2, 0 },
	{ PSF(weaponold[2]), 32, 0 }, { PSF(vLadderVec[2]), 0, 0 }, { PSF(weaponRestrictKickTime), -16, 0 },
	{ PSF(delta_angles[2]), -100, 0 }, { PSF(spreadOverride), 6, 0 }, { PSF(spreadOverrideState), 2, 0 },
	{ PSF(actionSlotParam[0]), 7, 0 }, { PSF(actionSlotParam[1]), 7, 0 }, { PSF(dofNearStart), 0, 0 },
	{ PSF(dofNearEnd), 0, 0 }, { PSF(dofFarStart), 0, 0 }, { PSF(dofFarEnd), 0, 0 }, { PSF(dofViewmodelStart), 0, 0 },
	{ PSF(dofViewmodelEnd), 0, 0 }, { PSF(viewAngleClampBase[0]), 0, 0 }, { PSF(weaponrechamber[1]), 32, 0 },
	{ PSF(weaponrechamber[2]), 32, 0 }, { PSF(weaponrechamber[3]), 32, 0 }, { PSF(leanf), 0, 0 },
...
static netField_t objectiveFields[] = { { OBJF(origin[0]), 0, 0 }, { OBJF(origin[1]), 0, 0 }, { OBJF(origin[2]), 0, 0 },
	{ OBJF(icon), 12, 0 }, { OBJF(entNum), 10, 0 }, { OBJF(teamNum), 4, 0 } };
...
void MSG_WriteDeltaObjective(struct snapshotInfo_s *snapInfo, msg_t *msg, int time, objective_t *from, objective_t *to)
{
	...
	for (i = 0, field = objectiveFields; i < numStateFields;
		 i++, field++) // Write out all fields in case a single one has changed
	{
...
static netField_t hudElemFields[] = { { HEF(color.rgba), -85, 0 }, { HEF(fadeStartTime), -97, 0 },
	{ HEF(fromColor.rgba), -85, 0 }, { HEF(y), -91, 0 }, { HEF(type), 4, 0 }, { HEF(materialIndex), 8, 0 },
	{ HEF(height), 10, 0 }, { HEF(width), 10, 0 }, { HEF(x), -92, 0 }, { HEF(fadeTime), 16, 0 }, { HEF(z), -90, 0 },
	{ HEF(value), 0, 0 }, { HEF(alignScreen), 6, 0 }, { HEF(sort), 0, 0 }, { HEF(alignOrg), 4, 0 },
	{ HEF(offscreenMaterialIdx), 8, 0 }, { HEF(fontScale), -86, 0 }, { HEF(text), 9, 0 }, { HEF(font), 4, 0 },
	{ HEF(scaleStartTime), -97, 0 }, { HEF(scaleTime), 16, 0 }, { HEF(fromWidth), 10, 0 }, { HEF(fromHeight), 10, 0 },
	{ HEF(targetEntNum), 10, 0 }, { HEF(glowColor.rgba), -85, 0 }, { HEF(fxBirthTime), -97, 0 }, { HEF(soundID), 5, 0 },
	{ HEF(fxLetterTime), 12, 0 }, { HEF(fxDecayStartTime), 16, 0 }, { HEF(fxDecayDuration), 16, 0 },
	{ HEF(flags), 3, 0 }, { HEF(label), 9, 0 }, { HEF(time), -97, 0 }, { HEF(moveStartTime), -97, 0 },
	{ HEF(moveTime), 16, 0 }, { HEF(fromX), -99, 0 }, { HEF(fromY), -99, 0 }, { HEF(fromAlignScreen), 6, 0 },
	{ HEF(fromAlignOrg), 4, 0 }, { HEF(duration), 32, 0 } };

void MSG_WriteDeltaHudElems(struct snapshotInfo_s *snapInfo, msg_t *msg, const int time, hudelem_t *from, hudelem_t *to,
	int count)
{
...
qboolean MSG_ShouldSendPSField(struct snapshotInfo_s *snapInfo, byte sendOriginAndVel, playerState_t *ps,
	playerState_t *oldPs, netField_t *field)
{
	int *fromF, *toF;

	if (field->bits == -87)
	{
		if (snapInfo->archived || ps->otherFlags & 2 || ((oldPs->eFlags & 0xFF) ^ (ps->eFlags & 0xFF)) & 2
			|| ps->viewlocked_entNum != 1023 || ps->pm_type == 5)
		{
			return 1;
		}
		return 0;
...
void MSG_WriteDeltaPlayerstate(struct snapshotInfo_s *snapInfo, msg_t *msg, const int time, playerState_t *from,
	playerState_t *to)
{
	qboolean forceSend;
	...
	else
	{
		...
		*/
		if (from && svsHeader.clientArchive && MSG_WithinAllowedPredictionError(dist, to)
			&& predictedTime == to->commandTime)
		{
			sendOriginAndVel = 0;
			MSG_WriteBit0(msg);
		}
	...
	// ammo in clip
	for (j = 0; j < 8; j++)
	{ //----(Ninja)	modified for 128 weaps (CoD4)
		clipbits = 0;
		for (i = 0; i < 16; i++)
		{
...
netField_t clientStateFields[] = { { CSF(modelindex), 9, 0 }, { CSF(netname[0]), 32, 0 }, { CSF(rank), 8, 0 },
	{ CSF(prestige), 8, 0 }, { CSF(team), 2, 0 }, { CSF(attachedVehEntNum), 10, 0 }, { CSF(netname[4]), 32, 0 },
	{ CSF(attachModelIndex[0]), 9, 0 }, { CSF(netname[8]), 32, 0 }, { CSF(perks), 32, 0 }, { CSF(netname[12]), 32, 0 },
	{ CSF(attachModelIndex[1]), 9, 0 }, { CSF(maxSprintTimeMultiplier), 0, 0 }, { CSF(attachedVehSlotIndex), 2, 0 },
	{ CSF(attachTagIndex[5]), 5, 0 }, { CSF(attachTagIndex[0]), 5, 0 }, { CSF(attachTagIndex[1]), 5, 0 },
	{ CSF(attachTagIndex[2]), 5, 0 }, { CSF(attachTagIndex[3]), 5, 0 }, { CSF(attachTagIndex[4]), 5, 0 },
	{ CSF(attachModelIndex[2]), 9, 0 }, { CSF(attachModelIndex[3]), 9, 0 }, { CSF(attachModelIndex[4]), 9, 0 },
	...
	clientState_t *to, qboolean force)
{
	...
	if (to)
	{
		MSG_WriteDeltaStruct(snapInfo, msg, time, (const byte *)from, (const byte *)to, force, numFields,
			GetMinBitCount(MAX_CLIENTS - 1), clientStateFields, 1);
	}
	else
	{
...
unsigned int kbitmask[33] = { 0, 1, 3, 7, 15, 31, 63, 127, 255, 511, 1023, 2047, 4095, 8191, 16383, 32767, 65535,
...
void __cdecl MSG_ReadDeltaUsercmdKey(msg_t *msg, int key, usercmd_t *from, usercmd_t *to)
{
	...
	if (MSG_ReadKey(msg, key, 1))
	{
		...
		if (to->buttons & 4)
		{
			//        to->meleeChargeYaw = (float)MSG_ReadDeltaKeyShort(msg, key, (int16_t)(signed
			//        int)(from->meleeChargeYaw * 182.04445 + 0.5)) * 0.0054931641;
			to->meleeChargeYaw =
				(float)MSG_ReadDeltaKeyShort(msg, key, (int16_t)ANGLE2SHORT(from->meleeChargeYaw)) * 0.0054931641;
...
float MSG_ReadOriginFloat(msg_t *msg, int bits, float oldValue)
{
  ...
  if ( MSG_ReadBit(msg) )
  {
	...
	return (double)((((signed int)oldValue - coord + (1 << (24 - 1))) ^ MSG_ReadBits(msg, 24)) + coord - (1 << (24 -
1)));
  }
...
float MSG_ReadOriginZFloat(msg_t *msg, float oldValue)
{
  ...
  if ( MSG_ReadBit(msg) )
  {
	...
	return (double)((((signed int)oldValue - coord + (1 << (20 - 1))) ^ MSG_ReadBits(msg, 20)) + coord - (1 << (20 -
1)));
  }
  return (double)(MSG_ReadBits(msg, 7) - 64) + oldValue;
}
...
void MSG_ReadDeltaField(msg_t *msg, int time, const void *from, const void *to, netField_t *field, int print,
	bool noXor)
{
	byte *fromF;
	byte *toF;
...
netField_t archivedEntityFields[] = { { AEF(r.absmin[1]), 0, 0 }, { AEF(r.absmax[1]), 0, 0 },
	{ AEF(r.absmin[0]), 0, 0 }, { AEF(r.absmax[0]), 0, 0 }, { AEF(s.lerp.pos.trBase[1]), 0, 0 },
	{ AEF(s.lerp.pos.trBase[0]), 0, 0 }, { AEF(r.absmax[2]), 0, 0 }, { AEF(s.lerp.pos.trBase[2]), 0, 0 },
	{ AEF(r.absmin[2]), 0, 0 }, { AEF(s.groundEntityNum), 10, 0 }, { AEF(s.lerp.apos.trBase[1]), 0, 0 },
	{ AEF(s.eType), 8, 0 }, { AEF(s.lerp.apos.trBase[0]), 0, 0 }, { AEF(s.clientNum), 7, 0 },
	{ AEF(s.lerp.apos.trBase[2]), 0, 0 }, { AEF(s.lerp.eFlags), -98, 0 }, { AEF(r.svFlags), 32, 0 },
	{ AEF(s.events[0]), 8, 0 }, { AEF(s.eventSequence), 8, 0 }, { AEF(s.index), 10, 0 }, { AEF(s.legsAnim), 10, 0 },
	{ AEF(s.events[1]), -94, 0 }, { AEF(s.events[2]), -94, 0 }, { AEF(s.events[3]), -94, 0 }, { AEF(s.weapon), 7, 0 },
	{ AEF(s.weaponModel), 4, 0 }, { AEF(s.lerp.pos.trType), 8, 0 }, { AEF(s.lerp.apos.trType), 8, 0 },
	{ AEF(s.iHeadIcon), 4, 0 }, { AEF(s.iHeadIconTeam), 2, 0 }, { AEF(s.solid), 24, 0 },
	{ AEF(s.eventParms[0]), -93, 0 }, { AEF(s.torsoAnim), 10, 0 }, { AEF(s.lerp.pos.trTime), -97, 0 },
	{ AEF(s.lerp.pos.trDelta[0]), 0, 0 }, { AEF(s.lerp.pos.trDelta[1]), 0, 0 }, { AEF(s.lerp.pos.trDelta[2]), 0, 0 },
	{ AEF(s.otherEntityNum), 10, 0 }, { AEF(s.eventParms[1]), -93, 0 }, { AEF(s.surfType), 8, 0 },
	{ AEF(s.eventParm), -93, 0 }, { AEF(s.eventParms[2]), -93, 0 }, { AEF(s.un1), 8, 0 },
	{ AEF(s.eventParms[3]), 8, 0 }, { AEF(s.lerp.pos.trDuration), 32, 0 }, { AEF(s.fWaistPitch), 0, 0 },
	{ AEF(s.fTorsoPitch), 0, 0 }, { AEF(s.lerp.apos.trTime), -97, 0 }, { AEF(s.lerp.apos.trDelta[0]), 0, 0 },
	{ AEF(s.lerp.apos.trDelta[2]), 0, 0 }, { AEF(r.clientMask[0]), 32, 0 }, { AEF(r.clientMask[1]), 32, 0 },
	{ AEF(s.lerp.apos.trDelta[1]), 0, 0 }, { AEF(s.attackerEntityNum), 10, 0 }, { AEF(s.time2), -97, 0 },
	{ AEF(s.un2), 32, 0 }, { AEF(s.lerp.apos.trDuration), 32, 0 }, { AEF(s.loopSound), 8, 0 },
	{ AEF(s.lerp.u.anonymous.data[0]), 32, 0 }, { AEF(s.lerp.u.anonymous.data[1]), 32, 0 },
	{ AEF(s.lerp.u.anonymous.data[2]), 32, 0 }, { AEF(s.lerp.u.anonymous.data[3]), 32, 0 },
	{ AEF(s.lerp.u.anonymous.data[4]), 32, 0 }, { AEF(s.lerp.u.anonymous.data[5]), 32, 0 },
	{ AEF(s.lerp.u.anonymous.data[6]), 32, 0 }, { AEF(s.partBits[0]), 32, 0 }, { AEF(s.partBits[1]), 32, 0 },
	{ AEF(s.partBits[2]), 32, 0 }, { AEF(s.partBits[3]), 32, 0 } };

bool __cdecl MSG_WriteDeltaArchivedEntity(snapshotInfo_t *snapInfo, msg_t *msg, const int time, archivedEntity_t *from,
	archivedEntity_t *to, enum DeltaFlags flags)
{
	...
	*/
	...
	if (MSG_WriteDeltaStruct(snapInfo, msg, time, (byte *)from, (byte *)to, flags == DELTA_FLAGS_FORCE, numFields, 10,
...
int __cdecl MSG_ReadDeltaStruct(msg_t *msg, const int time, const void *from, void *to, unsigned int number,
...
int MSG_ReadDeltaClient(msg_t *msg, const int time, clientState_t *from, clientState_t *to, int number)
{
	...
	return MSG_ReadDeltaStruct(msg, time, (byte *)from, (byte *)to, number, numFields, GetMinBitCount(MAX_CLIENTS - 1),
		clientStateFields);
}
	...
	playerState_t *to, bool predictedFieldsIgnoreXor)
{
	...
	*/
	...
	{
#ifndef DEDICATEDONLY
		if (!CL_GetPredictedOriginForServerTime(CL_GetLocalClientGlobals(localClientNum), to->commandTime, to->origin,

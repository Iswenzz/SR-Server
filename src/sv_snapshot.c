...
__cdecl void SV_WriteSnapshotToClient(client_t *client, msg_t *msg)
{
	...
	else
	{
		...
		{
			Com_DPrintWarning(CON_CHANNEL_SERVER,
				"%s: Delta request from out of date entities - delta against entity %i, oldest is %i, current is %i.  "
				"Their old snapshot had %i entities in it\n",
				client->name, oldframe->first_entity, svs.nextSnapshotEntities - svs.numSnapshotEntities,
		...
		{
			Com_DPrintWarning(CON_CHANNEL_SERVER,
				"%s: Delta request from out of date clients - delta against client %i, oldest is %i, current is %i.  "
				"Their old snapshot had %i clients in it\n",
				client->name, oldframe->first_client, svs.nextSnapshotClients - svs.numSnapshotClients,
	...
	while (newindex < frame->num_entities || oldindex < from_num_entities)
	{
		...
		if (newnum < oldnum)
		{
			...
			if (SR_DemoIsPlaying(client))
				SR_DemoUpdateEntity(client, &snapInfo, msg, svsHeader.time, &svsHeader.svEntities[newnum].baseline.s,
					newent, qtrue);
			else
...
static void SV_AddCachedEntitiesVisibleFromPoint(int from_num_entities, int from_first_entity, float *origin,
	signed int clientNum, snapshotEntityNumbers_t *eNums)
{
	int e, i, l;
	int clientcluster;
	...
	for (e = 0; e < from_num_entities; e++)
	{
		...
		boxleafnums = CM_BoxLeafnums(aent->r.absmin, aent->r.absmax, clusternums,
			sizeof(clusternums) / sizeof(clusternums[0]), &lastLeaf);
		if (!boxleafnums)
		{
			continue;
		}
		...
		if (!(fogOpaqueDistSqrd == 0.0
				|| BoxDistSqrdExceeds(aent->r.absmin, aent->r.absmax, origin, fogOpaqueDistSqrd) == qfalse))
		{
			continue;
		}
		// add it
...
static cachedSnapshot_t *SV_GetCachedSnapshot(int *pArchiveTime)
{
	...
	if (archivedFrame < svs.nextArchivedSnapshotFrames - NUM_ARCHIVED_FRAMES)
	{
		...
		*pArchiveTime =
			((uint64_t)1000
				* (uint64_t)(svs.nextArchivedSnapshotFrames - (svs.nextArchivedSnapshotFrames - NUM_ARCHIVED_FRAMES)))
			/ (uint64_t)sv_fps->integer;
	}
...
static void SV_BuildClientSnapshot(client_t *client)
{
	...
	if (cachedSnap)
	{
		...
		SV_AddCachedEntitiesVisibleFromPoint(cachedSnap->num_entities, cachedSnap->first_entity, org, clientNum,
			&entityNumbers);

		for (i = 0; i < entityNumbers.numSnapshotEntities; ++i)
		{
			entState = &svs.snapshotEntities[svs.nextSnapshotEntities % svs.numSnapshotEntities];
			aent = &svs.cachedSnapshotEntities[(cachedSnap->first_entity + entityNumbers.snapshotEntities[i])
				% maxCachedSnapshotEntities];
	...
	for (snapClient = svs.clients, i = 0; i < sv_maxclients->integer; i++, snapClient++)
	{
		if (snapClient->state < CS_PRIMED
			|| (!snapClient->gamestateSent && snapClient->netchan.remoteAddress.type != NA_BOT))
		{
			continue;
		}
...
void SV_SendClientMessages(void)
{
	...
	svs.archivedSnapshotFrames[svs.nextArchivedSnapshotFrames % maxArchivedSnapshotFrames].start =
...
void __cdecl SV_GetClientPositionsFromCachedSnap(cachedSnapshot_t *cachedFrame, vec3_t *pos, vec3_t *angles,
	bool *success)
{
...
void SV_ArchiveSnapshot(msg_t *msg)
{
	...
	for (i = svsHeader.nextCachedSnapshotFrames - 1; i >= n; --i)
	{
		...
		if (cachedFrame->archivedFrame >= svsHeader.nextArchivedSnapshotFrames - svsHeader.fps
			&& !cachedFrame->usesDelta)
		{
			if (cachedFrame->first_entity >= svsHeader.nextCachedSnapshotEntities - svsHeader.numCachedSnapshotEntities
				&& cachedFrame->first_client
					>= svsHeader.nextCachedSnapshotClients - svsHeader.numCachedSnapshotClients)
			{
				MSG_WriteBit0(msg);
				...
				MSG_ClearLastReferencedEntity(msg);
				/*
						from = &svsHeader.cachedSnapshotMatchStates[cachedFrame->matchState %
				   svsHeader.numCachedSnapshotMatchStates]; matchState = G_GetMatchStateLocal();
				...
				*/
				...
				for (e = 0; e < svsHeader.num_entities; ++e)
				{
					...
					if (gent->r.linked)
					{
						assertx(gent->s.number == e, "entnum: %d vs %d, eType: %d, origin: %f %f %f", gent->s.number, e,
							gent->s.eType, gent->r.currentOrigin[0], gent->r.currentOrigin[1],
...
#ifndef NDEBUG
							assertx(lastEntityNum != gent->s.number, "lastEntityNum is %i, cur entnum is %i",
	...
	svsHeader.cachedSnapshotFrames[svsHeader.nextCachedSnapshotFrames % 512].archivedFrame =
		svsHeader.nextArchivedSnapshotFrames;
	...
	*/
	...
	for (i = 0, client = svsHeader.clients; i < svsHeader.maxclients; ++i, ++client)
	{
		if (client->state >= CS_PRIMED)
		{
			cachedCl =
				&svsHeader
					 .cachedSnapshotClients[svsHeader.nextCachedSnapshotClients % svsHeader.numCachedSnapshotClients];
	...
	for (e = 0; e < svsHeader.num_entities; ++e)
	{
		...
		{
			assertx(gent->s.number == e, "entnum: %d vs %d, eType: %d, origin: %f %f %f", gent->s.number, e,
				gent->s.eType, gent->r.currentOrigin[0], gent->r.currentOrigin[1], gent->r.currentOrigin[2]);
				...
				/*&& !(gent->r.svFlags & 0x80)*/)
			{
				...
				archEnt = &svsHeader.cachedSnapshotEntities[svsHeader.nextCachedSnapshotEntities
...
cachedSnapshot_t *SV_GetCachedSnapshotInternal(int archivedFrame, int depth, bool expectedToSucceed)
{
	...
	for (i = svs.nextCachedSnapshotFrames - 1; i >= firstCachedSnapshotFrame; --i)
	{
		...
		if (cachedFrame->archivedFrame == archivedFrame)
		{
			...
			if (/* cachedFrame->matchState > depth + svs.nextCachedSnapshotMatchStates -
		  svs.numCachedSnapshotMatchStates
		  && */
				cachedFrame->first_entity > 160 * depth + svs.nextCachedSnapshotEntities - svs.numCachedSnapshotEntities
				&& cachedFrame->first_client
					> 32 * depth + svs.nextCachedSnapshotClients - svs.numCachedSnapshotClients)
			{
				return cachedFrame;
			}
			break;
	...
	{
		MSG_InitReadOnlySplit(&msg, &svs.archivedSnapshotBuffer[startIndex], partSize, &svs.archivedSnapshotBuffer[0],
			frame->size - partSize);
	}
	else
	{
	...
	if (MSG_ReadBit(&msg))
	{
		...
		MSG_ClearLastReferencedEntity(&msg);
		/*
			MSG_ReadDeltaMatchState(&msg, cachedFrame->time, 0,
		...
		*/
		while (MSG_ReadBit(&msg))
		{
			...
			MSG_ReadDeltaClient(&msg, cachedFrame->time, 0,
				&svs.cachedSnapshotClients[svs.nextCachedSnapshotClients % svs.numCachedSnapshotClients].cs, newnum);
	...
	else
	{
		...
		if (oldArchivedFrame < svs.nextArchivedSnapshotFrames - NUM_ARCHIVED_FRAMES)
		{
			...
			{
				Com_Printf(CON_CHANNEL_SERVER,
					"getting archive snapshot failed for time %i - oldArchiveFrame(%i) < "
					"svs.nextArchivedSnapshotFrames(%i) - NUM_ARCHIVED_FRAMES(%i)\n",
		...
		if (!SV_FrameIsStillInArchivedSnapshotBuffer(frame->start))
		{
			...
			{
				Com_Printf(CON_CHANNEL_SERVER,
					"getting archive snapshot failed for time %i - frame->start(%i) < "
					"svs.nextArchivedSnapshotBuffer(%i) - ARCHIVEDSSBUF_SIZE(%i)\n",
					archivedFrame, frame->start, svs.nextArchivedSnapshotBuffer, ARCHIVEDSSBUF_SIZE);
				svs.nextArchivedSnapshotErrorTime = svs.time + 1000;
			}
			return 0;
		...
		if (!oldCachedFrame)
		{
			...
			{
				Com_Printf(CON_CHANNEL_SERVER,
					"failed to get snapshot for time %i - it was delta'd off time %i, and we couldn't get that "
					"snapshot\n",
		...
		while (MSG_ReadBit(&msg))
		{
			...
			if (oldnum == newnum)
			{
				...
				if (cachedClient->playerStateExists)
				{
					...
					{
						MSG_ReadDeltaPlayerstate(0, &msg, cachedFrame->time, &oldCachedClient->ps, &cachedClient->ps,
							0);
					}
					else
					{
			...
			else
			{
				...
				MSG_ReadDeltaClient(&msg, cachedFrame->time, 0,
					&svs.cachedSnapshotClients[svs.nextCachedSnapshotClients % svs.numCachedSnapshotClients].cs,
					newnum);
				cachedClient->playerStateExists = MSG_ReadBit(&msg);
				if (cachedClient->playerStateExists)
				{
	...
	assertx(cachedFrame->num_entities < svs.numCachedSnapshotEntities, "(cachedFrame->num_entities) = %i",
		cachedFrame->num_entities);
	assertx(cachedFrame->num_clients < svs.numCachedSnapshotClients, "(cachedFrame->num_clients) = %i",
		cachedFrame->num_clients);
	...
	int *health, int *otherFlags)
{
	...
	if (cachedSnapshot)
	{
		...
		for (i = 0;; ++i)
		{
			...
			cachedClient =
				&svs.cachedSnapshotClients[(i + cachedSnapshot->first_client) % svs.numCachedSnapshotClients];
		...
		if (ps)
		{
			...
			for (i = 0; i < 0x1F && ps->hud.archival[i].type; ++i)
			{
				...
				if (ps->hud.archival[i].fadeStartTime)
				{
					...
					assertx(ps->hud.archival[i].fadeStartTime <= svs.time, "%i, %i", ps->hud.archival[i].fadeStartTime,
						svs.time);
				}

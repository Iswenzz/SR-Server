...
void DB_ParseRequestedXAssetNum()
{
	...
	cvar_t *r_xassetnum = Cvar_RegisterString("r_xassetnum", "", CVAR_INIT,
		"The list of xassets with their count in the key=value key=value... format");

	Com_sprintf(toparse, sizeof(toparse), " %s", r_xassetnum->string);

	for (i = 0; i < ASSET_TYPE_COUNT; ++i)
	{
		if (DB_XAssetNoAlloc((XAssetType)i) || i == ASSET_TYPE_MENU || i == ASSET_TYPE_WEAPON
			|| i == ASSET_TYPE_STRINGTABLE)
		{
			continue;
		}
...
extern "C"
{
	...
	double __cdecl DB_GetLoadedFraction()
	{
		...
		return 0.5
			* ((loadedBytesInternal + loadedBytesExternal) / (totalBytesInternal + (double)g_totalExternalBytes));
	}
	...
	void __cdecl DB_AllocXBlocks(unsigned int *blockSize, const char *filename, XBlock *blocks, unsigned int allocType,
		int flags)
	{
			...
			if (size)
			{
				...
				if (!buf)
				{
					Com_Error(ERR_DROP,
						...
						(double)size * 0.00000095367432, g_block_mem_name[blockIndex], filename,
	...
	{
		//  *(DWORD*)data = (DWORD)&g_streamBlocks[(unsigned int)(*(DWORD *)data - 1) >> 29].data[(*(DWORD *)data - 1) &
		//  0x1FFFFFFF];
		*(DWORD *)data =
			(DWORD)&g_streamBlocks[(unsigned int)(*(DWORD *)data - 1) >> 28].data[(*(DWORD *)data - 1) & 0xFFFFFFF];
	}
	...
	void __stdcall DB_FileReadCompletionDummyCallback(long unsigned int dwErrorCode,
		long unsigned int dwNumberOfBytesTransfered, struct _OVERLAPPED *lpOverlapped)
	{
	}
		...
		while (size + g_load.deflateBufferPos > DEFLATE_BUFFER_SIZE && size > 0)
		{
			...
			do
			{
				...
				if (err && err != 1)
				{
					...
					DB_CancelLoadXFile();
					Com_Error(ERR_DROP, "Fastfile for zone '%s' appears corrupt or unreadable (code %i.)",
						g_load.filename, err + 110);
				}
				if (err == Z_STREAM_END)
				{
					if (g_load.stream.avail_out > 0 && g_load.ateof)
					{
						DB_CancelLoadXFile();
						Com_Error(ERR_DROP,
							"Fastfile for zone '%s' appears corrupt or unreadable. Unexpected end of stream. Missing "
							"%d bytes.",
				...
				if (g_load.f)
				{
					assert((unsigned int)(g_load.stream.next_in - (byte *)g_load.compressBufferStart)
						<= DBFILE_BUFFER_SIZE);
...
//          R_ShowDirtyDiscError();
	...
	void __cdecl DB_LoadXFileInternal()
	{
		...
		if (version != FASTFILE_VERSION)
		{
			if (version >= FASTFILE_VERSION)
			{
				Com_Error(ERR_DROP, "Fastfile for zone '%s' is newer than client executable (version %d, expecting %d)",
					g_load.filename, version, FASTFILE_VERSION);
			}
			else
			{
				Com_Error(ERR_DROP, "Fastfile for zone '%s' is out of date (version %d, expecting %d)", g_load.filename,
		...
		Com_Printf(CON_CHANNEL_FILES, "Loaded fastfile '%s' in %ims (%dms waiting)\n", g_load.filename,
			Sys_Milliseconds() - g_load.startTime, g_totalWait);
		if (g_load.flags & 1)
		{
	...
	bool __cdecl DB_LoadXFile(const char *path, HANDLE f, const char *filename, XBlock *blocks,
		void(__cdecl *interrupt)(), char *buf, int allocType, int flags)
	{
		//  assert ( (byte)buf & 3 );
	...
	int DB_BuildZoneFilePath(const char *zoneName, char *oFilename, int maxlen)
	{
		...
		if (fs_gameDirVar->string && fs_gameDirVar->string[0]) // Load from fs_gamedirvar or from zone/language
		{
			if (Q_stricmp(zoneName, "common_mp") == 0 || Q_stricmp(zoneName, "localized_common_mp") == 0
				|| Q_stricmp(zoneName, "code_post_gfx_mp") == 0
	...
	void __cdecl DB_Init()
	{
		...
		db_xassetdebugtype = Cvar_RegisterInt("db_xassetdebugtype", -2, -2, 43, 0,
			"debug assets loading type: -1 all; indexes start at 0");
		db_nobspweapon = Cvar_RegisterBool("db_nobspweapon", qfalse, 0,
			"Do not load weapons from map/usermap fastfiles. Can free up to 2 weapons");
	...
	void *__cdecl DB_FindXAssetDefaultHeaderInternal(XAssetType type)
	{
		...
		for (assetEntryIndex = db_hashTable[DB_HashForName(name, type) % 0x8000];;
...
void XAssetUsage_f()
{
	...
	for (assettype = 0; assettype < ASSET_TYPE_COUNT; assettype++)
	{
		...
		Com_Printf(CON_CHANNEL_DONT_FILTER, " %5d %5d\n", countlist[assettype],
			g_poolSize[assettype] - countlist[assettype]);
	}
	Com_Printf(CON_CHANNEL_DONT_FILTER, "\n");
}
		...
		for (assetEntryIndex = db_hashTable[hash]; assetEntryIndex; assetEntryIndex = assetEntry->entry.nextHash)
		{
			...
			if (g_assetEntryPool[assetEntryIndex].entry.asset.type == type)
			{
				Com_Printf(CON_CHANNEL_DONT_FILTER, "'%s','%s'\n",
					DB_GetXAssetHeaderName(type, &assetEntry->entry.asset.header),
					g_zones[assetEntry->entry.zoneIndex].zoneinfo.name);
				if (includeOverride)
				{
						 ...
						 overrideAssetEntryIndex = g_assetEntryPool[overrideAssetEntryIndex].entry.nextOverride)
					{
						Com_Printf(CON_CHANNEL_DONT_FILTER, "'%s','%s'\n",
							...
							g_zones[(uint8_t)g_assetEntryPool[overrideAssetEntryIndex].entry.zoneIndex].zoneinfo.name);
					}
				}
			}
		}
...
void __cdecl DB_PrintXAssetsForType(XAssetType type, void(__cdecl *func)(XAssetHeader, void *), void *inData,
...
extern "C"
{
	...
	void __cdecl Load_WeaponDefAsset(struct WeaponDef **weapon)
	{
		...
		{
			if (Q_stricmp(g_load.filename, "common_mp") && Q_stricmp(g_load.filename, "mod")
				&& Q_stricmp(g_load.filename, "ui_mp"))
			{
	...
	XAssetEntryPoolEntry *__cdecl DB_FindXAssetEntry(XAssetType type, const char *name)
	{
		...
		for (assetEntryIndex = db_hashTable[DB_HashForName(name, type) % ARRAY_COUNT(db_hashTable)]; assetEntryIndex;
			 assetEntryIndex = assetEntry->entry.nextHash)
		{
			assetEntry = &g_assetEntryPool[assetEntryIndex];
			if (g_assetEntryPool[assetEntryIndex].entry.asset.type == type)
		...
		if (!asset.header.xmodelPieces)
		{
			...
			if (type != ASSET_TYPE_CLIPMAP && type != ASSET_TYPE_CLIPMAP_PVS)
			{
				Com_Error(ERR_DROP, "Could not load default asset '%s' for asset type '%s'.\nTried to load asset '%s'.",
					g_defaultAssetName[type], DB_GetXAssetTypeName(type), name);
			}
			else
			{
				Com_Error(ERR_DROP,
	...
	bool __cdecl DB_IsXAssetDefault(XAssetType type, const char *name)
	{
		...
		Sys_EnterCriticalSection(CRITSECT_DBHASH);
		for (assetEntryIndex = db_hashTable[hash % ARRAY_COUNT(db_hashTable)]; assetEntryIndex;
			 assetEntryIndex = assetEntry->entry.nextHash)
		{
			assetEntry = &g_assetEntryPool[assetEntryIndex];
			if (g_assetEntryPool[assetEntryIndex].entry.asset.type == type)
	...
	void __cdecl DB_EnumXAssets_FastFile(XAssetType type, void(__cdecl *func)(void *header, void *indata), void *inData,
		bool includeOverride)
	{
		unsigned int hash;
		unsigned int assetEntryIndex;
	...
	void __cdecl DB_DelayedCloneXAsset(XAssetEntry *newEntry)
	{
		...
		if (g_sync)
		{
			if (db_xassetdebug->boolean)
			{
				...
				if (db_xassetdebugtype->integer == -1 || db_xassetdebugtype->integer == newEntry->asset.type
		...
		else
		{
			...
			{
				Com_Printf(CON_CHANNEL_DONT_FILTER,
					"g_copyInfo exceeded: too many asset overrides in one call to DB_LoadXAssets.\n");
				for (i = 0; i < 2048; ++i)
				{
					assetname = DB_GetXAssetName(&g_copyInfo[i]->asset);
					Com_Printf(CON_CHANNEL_DONT_FILTER, "'%s','%s'\n", DB_GetXAssetTypeName(g_copyInfo[i]->asset.type),
						assetname);
				}
			...
			if (db_xassetdebug->boolean)
			{
					...
					|| (debugname[0] && !Q_stricmp(debugname, assetname)))
				{
					...
					Com_Printf(CON_CHANNEL_SYSTEM,
						...
						DB_GetXAssetTypeName(newEntry->asset.type), assetname,
						g_zones[newEntry->zoneIndex].zoneinfo.name);
				}
			}
	...
	XAssetEntryPoolEntry *__cdecl DB_LinkXAssetEntry(XAssetEntry *argNewEntry, int allowOverride)
	{
		...
		hash = DB_HashForName(name, type);
		for (existingEntryIndex = db_hashTable[hash % ARRAY_COUNT(db_hashTable)]; existingEntryIndex;
				...
				if (!Q_stricmp(locname, name))
				{
					if (db_xassetdebug->boolean)
					{
						...
						if (db_xassetdebugtype->integer == -1
							|| db_xassetdebugtype->integer == newEntry->entry.asset.type
							|| (debugname[0] && !Q_stricmp(name, debugname)))
						{
			...
			if (db_xassetdebug->boolean)
			{
					...
					|| (debugname[0] && !Q_stricmp(name, debugname)))
				{
					...
					Com_Printf(CON_CHANNEL_SYSTEM,
						...
						g_zones[newEntry->entry.zoneIndex].zoneinfo.name);
				}
			}
			return newEntry;
		}
		...
		if (existingEntry->entry.zoneIndex)
		{
			...
			if (!g_defaultAssetName[type][0] && type != ASSET_TYPE_RAWFILE && type != ASSET_TYPE_MAP_ENTS)
			{
				...
				Com_Error(ERR_DROP, "Attempting to override asset '%s' from zone '%s' with zone '%s'", name,
					g_zones[existingEntry->entry.zoneIndex].zoneinfo.name,
					g_zones[newEntry->entry.zoneIndex].zoneinfo.name);
			}
			if (!DB_OverrideAsset(newEntry->entry.zoneIndex, existingEntry->entry.zoneIndex, type))
			{
				...
				if (db_xassetdebug->boolean)
				{
					...
					{
						Com_Printf(CON_CHANNEL_SYSTEM,
							"DB_LinkXAssetEntry: keep existing asset, and put new in its override position\n");
					}
				}
			...
			if (allowOverride)
			{
				...
				if (db_xassetdebug->boolean)
				{
						...
						|| (debugname[0] && !Q_stricmp(name, debugname)))
					{
						locname = DB_GetXAssetTypeName(newEntry->entry.asset.type);
						Com_Printf(CON_CHANNEL_SYSTEM,
							"DB_LinkXAssetEntry: swapping existing asset: '%s','%s' with new asset loaded from "
							"fastfile: '%s'\n",

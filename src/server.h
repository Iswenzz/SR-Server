...
{ // 0x8c51780
	...
	int timeResidual; // Used for remainder of the integer division of frametime / 1000 and prodive exact timing for
					  // svs.time
...
{ // 0x13e78d00
	...
	int frameusec; // Frameusec set every Level-startup to the desired value from sv_fps    Replaces: byte inFrame; byte
				   // smp; byte allowNetPackets;
...
extern "C"
{
	...
	char *SV_PlayerIsBanned(uint64_t playerid, uint64_t steamid, netadr_t *addr, const char *name, char *message,
		int len);
	...
	void __cdecl SV_Trace(trace_t *results, const float *start, const float *mins, const float *maxs, const float *end,
		IgnoreEntParams *ignoreEntParams, int contentmask, int locational, char *priorityMap,
		int staticmodels); // 0817D9F8
	...
	void G_TraceCapsule(trace_t *results, const float *start, const float *mins, const float *maxs, const float *end,
	...
	bool MSG_WriteDeltaArchivedEntity(snapshotInfo_t *snapInfo, msg_t *msg, const int time, archivedEntity_t *from,
		archivedEntity_t *to, enum DeltaFlags flags);
	int MSG_ReadDeltaArchivedEntity(msg_t *msg, const int time, archivedEntity_t *from, archivedEntity_t *to,
		int number);

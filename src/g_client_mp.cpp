...
void ClearAllSpectators(struct gentity_s* ent)
{
	...
	for (i = 0, followers = level.gentities; i < level.maxclients;
		 i++, followers++) // let refollow all spectors me who have prior followed me
	{
...
extern "C" void __cdecl ClientSpawn(gentity_s* ent, const float* spawn_origin, const float* spawn_angles)
{
	...
	*/
	...
	assertx((int)(client - level.clients) < level.maxclients,
		"client - level.clients doesn't index level.maxclients\n\t%i not in [0, %i)", client - level.clients,
		level.maxclients);
...
extern "C" __cdecl void ClientEndFrame_TurretThink_Stub(gentity_s* ent)
{
	...
	release_assertx(level.gentities[client->ps.viewlocked_entNum].r.ownerNum.isDefined()
			&& (level.gentities[client->ps.viewlocked_entNum].r.ownerNum.ent() == ent),
		"viewlocked_entNum is %i, ownerNum is %i, ent->s.number is %i", client->ps.viewlocked_entNum,
		level.gentities[client->ps.viewlocked_entNum].r.ownerNum, ent->s.number);

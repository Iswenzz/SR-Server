...
__optimize3 __regparm1 void SV_GetChallenge(netadr_t *from)
{
	...
	if (from->type == NA_IP && svs.authorizeAddress.type != NA_DOWN)
	{
		...
		if (svs.authorizeAddress.type == NA_BAD)
		{
			...
			if (NET_StringToAdr(AUTHORIZE_SERVER_NAME, &svs.authorizeAddress, NA_IP))
			{
				...
				Com_Printf(CON_CHANNEL_SERVER, "%s resolved to %s\n", AUTHORIZE_SERVER_NAME,
					NET_AdrToString(&svs.authorizeAddress));
			}
			else
			{
		...
		if (svs.authorizeAddress.type == NA_IP && from->type == NA_IP
			&& NET_CompareBaseAdr(from, &svs.authorizeAddress))
		{
...
__optimize3 __regparm1 void SV_DirectConnect(netadr_t *from)
{
			...
			&& (cl->netchan.qport == qport || from->port == cl->netchan.remoteAddress.port))
		{
			...
			if (cl->state == CS_CONNECTED)
			{
				Com_DPrintf(CON_CHANNEL_SERVER,
					"Rejected connection from %s. DoS attack detected by emulating client 1.8\n",
			...
			&& (cl->state == CS_CONNECTED || cl->state == CS_ZOMBIE))
		{
	...
	{
		if (!sv_steamgroup || !sv_steamgroup->string[0]) // If server is in a Steamgroup we have to let him in and see
														 // later if he is a member of steam group
		{
	...
	// Process queue
	for (i = 0; i < 10; i++)
	{ // Purge all older players from queue
	...
	for (i = 0; i < 10; i++)
	{ // Move waiting players up in queue if there is a purged slot
	...
	for (i = 0; i < 10; i++)
	{ // Find highest slot or the one which is already assigned to this player
...
__optimize3 __regparm2 void SV_ReceiveStats(netadr_t *from, msg_t *msg)
{
	...
	{
		Com_DPrintf(CON_CHANNEL_SERVER,
			"SV_ReceiveStats: Received statspacket from disconnected remote client: %s qport: %d\n",
...
void SV_ReceiveStats_f(client_t *cl, msg_t *msg)
{
	...
	else
	{
		...
		uint8_t iv[16] = { 0x4f, 0x11, 0x62, 0xeb, 0x44, 0x61, 0x99, 0x66, 0xa4, 0xcf, 0x41, 0x73, 0x99, 0x12, 0x55,
			0xb9 };
...
__cdecl void SV_DropClientInternal(client_t *drop, const char *reason, qboolean notifyOthers)
{
	...
	if (drop->steamid > 0)
	{
		...
		SV_EnterLeaveLog("^4Client %s %s ^4left this server from slot %d with steamid %s playerid %s", drop->name,
			NET_AdrToString(&drop->netchan.remoteAddress), clientnum, ssti, psti);
	}
	else
		SV_EnterLeaveLog("^4Client %s %s ^4left this server from slot %d with playerid %s", drop->name,
			NET_AdrToString(&drop->netchan.remoteAddress), clientnum, psti);
	...
	drop->reliableSent = drop->reliableSequence =
		drop->reliableAcknowledge; // Reset unsentBuffer and Sequence to ommit the outstanding junk from beeing
								   // transmitted
...
void SV_ClientEnterWorld(client_t *client, usercmd_t *cmd)
{
	...
	// SV_SpawnServer() stops the demo and cleans the name which did not happen here which resulted in strange naming
	// bug

	if (sv_autodemorecord->boolean && !client->demorecording
		&& (client->netchan.remoteAddress.type == NA_IP || client->netchan.remoteAddress.type == NA_IP6))
	{
		...
		&& (client->netchan.remoteAddress.type == NA_IP || client->netchan.remoteAddress.type == NA_IP6))
	{
		if (client->steamid != 0)
		{
			...
			SV_EnterLeaveLog("^5Client %s %s ^5entered this server in slot %d with steamid %s playerid %s",
				client->name, NET_AdrToString(&client->netchan.remoteAddress), clientNum, ssti, psti);
			client->power = Auth_GetClPowerBySteamID(client->steamid);
		}
		else
		{
			SV_EnterLeaveLog("^5Client %s %s ^5entered this server in slot %d with playerid %s", client->name,
				NET_AdrToString(&client->netchan.remoteAddress), clientNum, psti);
		}
	}
...
gentity_t *SV_AddBotClient(char *requested_name)
{
	...
	*userinfo = 0;
	...
	// init the netchan queue
	Netchan_Setup(NS_SERVER, &cl->netchan, botnet, qport, cl->unsentBuffer, sizeof(cl->unsentBuffer),
	...
	/*	for(index = 0; index < MAX_RELIABLE_COMMANDS; index++ ){
			if(index < MAX_RELIABLE_COMMANDS / 2){
				...
				cl->reliableCommands[index] = &svs.extclients[i].highReliableCommands[index & (MAX_RELIABLE_COMMANDS -
	   1)];
			}
		}
	*/
...
__cdecl void SV_WriteDownloadToClient(client_t *cl)
{
	...
	if (!cl->download)
	{
			...
			|| (cl->downloadSize = FS_SV_FOpenFileRead(cl->downloadName, &cl->download)) <= 0)
		{
			...
			if (!sv_allowDownload->integer)
			{
				Com_Printf(CON_CHANNEL_SERVER, "clientDownload: %d : \"%s\" download disabled", cl - svs.clients,
					cl->downloadName);
...
void SV_SendClientGameState(client_t *client)
{
	...
	PHandler_Event(PLUGINS_ONPLAYERGOTAUTHINFO, &client->netchan.remoteAddress, &playerid, &steamid, banrejectmsg,
		&returnNow, client);
	if (returnNow)
	{
		return;
	...
	if (banrejectmsg[0]
		|| SV_PlayerIsBanned(client->playerid, client->steamid, &client->netchan.remoteAddress, client->name,
			banrejectmsg, sizeof(banrejectmsg)))
	{
	...
	if (client->gamestateMessageNum
		== -1) // It is the 1st time he connects to this server and receives a gamestate. Limit that to this event
	{
		SV_UpdateClientConfigInfo(
			client); // Let everyone know about this client additional information (Clantag + Username atm.)
...
__optimize3 __regparm2 void SV_ExecuteClientMessage(client_t *cl, msg_t *msg)
{
	...
	decompressMsg.cursize = MSG_ReadBitsCompress(msg->data + msg->readcount, msg->cursize - msg->readcount,
...
void SV_WWWDownload_ChkFail_f(client_t *cl)
{
	...
	Com_PrintWarning(CON_CHANNEL_SERVER,
		"Client '%s' reports that the redirect download for '%s' had wrong checksum.\n        You should make sure "
		"that your files on your redirect are the same files you have on your server\n",
...
void SV_SelectDownloadBlocksX_f(client_t *cl, msg_t *msg)
{
	...
	{
		Com_DPrintf(CON_CHANNEL_SERVER,
			"SV_SelectDownloadBlocksX_f a serverdownload is no longer in progress - ignoring this command\n");
		return;
	}
...
client_t *SV_ReadPackets(netadr_t *from, unsigned short qport)
{
	...
	for (i = 0, cl = svs.clients; i < sv_maxclients->integer; i++, cl++)
	{
		...
		if (cl->netchan.remoteAddress.port != from->port)
		{
			...
			// became terrible with their CGNAT - we don't wanna see this anymore
...
void SV_ReceiveReliableMessages(client_t *client)
{
	...
	while (1)
	{
		...
		{ // Incomplete message
			...
			msg->cursize += ReliableMessageReceive(client->reliablemsg.netstate, msg->data + msg->cursize,
				(messagesize + 4) - msg->cursize);
			if (msg->cursize < messagesize + 4)
			{
				return;
			}
...
qboolean SV_SetupReliableMessageProtocol(client_t *client)
{
	...
	client->reliablemsg.netstate =
		ReliableMessageSetup(client->netchan.qport, client->netchan.sock, &client->netchan.remoteAddress);

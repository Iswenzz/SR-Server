...
void SV_DumpReliableCommands( client_t *client, const char* cmd) {
	...
	Com_sprintf(msg, sizeof(msg), "Cl: %i, Seq: %i, Time: %i, NotAck: %i, Len: %i, Msg: %s\n",
		client - svs.clients, client->reliableSequence, svs.time ,client->reliableSequence -
client->reliableAcknowledge, strlen(cmd), cmd);
		...
		if (client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)].cmdType)
		{
			if ((v1 & (MAX_RELIABLE_COMMANDS - 1)) != (i & (MAX_RELIABLE_COMMANDS - 1)))
			{
				memcpy(&client->reliableCommands[v1 & (MAX_RELIABLE_COMMANDS - 1)],
					&client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)], sizeof(reliableCommands_t));
			}
			++v1;
		}
	}
...
int sub_530FC0(client_t *client, const char *command)
{
	...
	for (i = client->reliableSent + 1; i <= client->reliableSequence; ++i)
	{
		...
		case 100:
		case 118:
			if (!I_IsEqualUnitWSpace((char *)&command[2],
					&client->reliableCommands[i & (MAX_RELIABLE_COMMANDS - 1)].command[2]))
			{
				continue;
			}
		case 67:
...
void __cdecl SV_AddServerCommand(client_t *client, int type, const char *cmd)
{
	...
	else
	{
		...
		{
			memcpy(&client->reliableCommands[v4 & 0x7F], &client->reliableCommands[i++ & 0x7F],
	...
	MSG_WriteReliableCommandToBuffer(cmd, client->reliableCommands[index].command,
...
__optimize3 __regparm3 static qboolean SVC_RateLimitAddress(netadr_t *from, int burst, int period)
{
	...
	{ // Init error, to be save we deny everything
		return qtrue;
	}
}
...
void SVC_SourceEngineQuery_Player(netadr_t *from, msg_t *recvmsg)
{
	...
	for (i = 0, cl = svs.clients, gclient = level.clients, numClients = 0; i < sv_maxclients->integer;
		 i++, gclient++, cl++)
	{
...
__optimize3 __regparm2 void SV_PacketEvent(netadr_t *from, msg_t *msg)
{
		...
		|| (cl->reliableSequence - cl->reliableAcknowledge) < 0)
	{
		Com_Printf(CON_CHANNEL_SERVER,
			"Out of range reliableAcknowledge message from %s - reliableSequence is %i, reliableAcknowledge is %i\n",
...
void SV_HeartBeatMessageLoop(msg_t *msg, qboolean authoritative, qboolean *needticket, char *challenge)
{
	...
	while (msg->readcount < msg->cursize)
	{
		...
		case 1:
			...
			{
				Com_PrintError(CON_CHANNEL_SERVER,
					"Failure registering server on masterserver. Server address is banned: %s\n",
			...
			break;
		case 2:
			// Remote enforced restart (Usually for protocol mismatch) (Servers which don't respond to this are usually
			// rendered obsolete soon after a new protocol update)
			ic = MSG_ReadLong(&singlemsg);
			if (ic == 1)
			{
				MSG_ReadString(&singlemsg, stringline, sizeof(stringline));
				...
				{
					Com_Printf(CON_CHANNEL_SERVER,
						"Received restart message from masterserver which is not authoritative. Ignoring\n");
				}
			}
			break;
		case 3:
			...
			if (ic == 1)
			{
				...
				{
					Com_Printf(CON_CHANNEL_SERVER,
						"Received broadcast message from masterserver which is not authoritative. Ignoring\n");
				}
			}
			break;
		default:
...
void SV_SendReceiveHeartbeatTCP(netadr_t *adr, netadr_t *sourceadr, byte *message, int qlen, qboolean authoritative,
	qboolean *needticket, char *challenge)
{
	int l = 0;
	byte response[16384];
	char line[256];
	...
	for (i = 0; i < count; ++i)
	{
		...
		if (iplist[i].type == NA_IP && opts->adr4.type == NA_IP && iplist[i].ip[0] != 127 && iplist[i].ip[0] < 224)
		{
			...
			SV_SendReceiveHeartbeatTCP(&opts->adr4, &iplist[i], opts->message, opts->messagelen, opts->authoritative,
				opts->needticket, opts->challengei4);
		}
		else if (iplist[i].type == NA_IP6 && opts->adr6.type == NA_IP6 && iplist[i].ip6[0] < 0xfe)
		{
			...
			SV_SendReceiveHeartbeatTCP(&opts->adr6, &iplist[i], opts->message, opts->messagelen, opts->authoritative,
				opts->needticket, opts->challengei6);
		}
	}
...
void SV_CreateAndSendMasterheartbeatMessage(const char *message, masterserver_t *masrv)
{
	...
	if (masrv->needticket)
	{
		...
		{
			Com_Printf(CON_CHANNEL_SERVER,
				"Can not register server on the masterserver. Server needs to provide a valid token in cvar "
				"sv_authtoken.\n");
			opts->locked = qfalse;
		...
		if (strlen(masterservers.servers[i].name) > 3)
		{
			...
			if (res)
			{
				Com_Printf(CON_CHANNEL_SERVER, "%s resolved to %s\n", masterservers.servers[i].name,
					NET_AdrToString(&masterservers.servers[i].i4));
			}
			else
			{
			...
			{
				Com_Printf(CON_CHANNEL_SERVER, "%s resolved to %s\n", masterservers.servers[i].name,
					NET_AdrToString(&masterservers.servers[i].i6));
			}
			else
			{
...
void serverStatus_Write()
{
	...
	XML_OpenTag(&xmlbase, "Game", 9, "CapatureLimit", "", "FragLimit", "", "Map", sv_mapname->string, "MapTime", "",
		"Name", "cod4", "RoundTime", "", "Rounds", "", "TimeLimit", "", "Type", sv_g_gametype->string);
	...
	for (i = 0, cl = svs.clients, gclient = level.clients; i < sv_maxclients->integer; i++, cl++, gclient++)
	{
		if (cl->state >= CS_CONNECTED)
		{
			...
			XML_OpenTag(&xmlbase, "Client", 15, "CID", cid, "ColorName", cl->name, "DBID", ssti, "IP",
				NET_AdrToStringShort(&cl->netchan.remoteAddress), "ID", psti, "Score", score, "Kills", kills, "Deaths",
				deaths, "Assists", assists, "Ping", ping, "Team", team, "TeamName", teamname, "Updated", timestr,
				"power", power, "rank", rank);
			XML_CloseTag(&xmlbase);
		}
	}
...
void SV_InitServerId()
{
	...
	/*    int read;
		...
		*buf = 0;
		...
		Q_strncpyz(psvs.masterServer_challengepassword, Info_ValueForKey(buf, "challenge_password"),
	   sizeof(psvs.masterServer_challengepassword));
...
void SV_InitCvarsOnce(void)
{
	...
	sv_queryIgnoreMegs = Cvar_RegisterInt("sv_queryIgnoreMegs", 1, 0, 32, 0x11,
		"Number of megabytes of RAM to allocate for the querylimit IP-blacklist. 0 disables this feature");
	sv_queryIgnoreTime = Cvar_RegisterInt("sv_queryIgnoreTime", 2000, 100, 100000, 1,
		"How much milliseconds have to pass until another two queries are allowed");
	Cvar_RegisterBool("sv_disableClientConsole", qfalse, 4,
	...
	sv_allowDownload =
		Cvar_RegisterBool("sv_allowDownload", qtrue, 1, "Allow clients to download gamefiles from server");
	sv_maxDownloadRate = Cvar_RegisterInt("sv_maxDownloadRate", 1024, 128, 8192, 0,
		"Rate in kilobytes all clients can together receive when downloading from server");
	sv_wwwDownload = Cvar_RegisterBool("sv_wwwDownload", qfalse, 1, "Enable http download");
	sv_wwwBaseURL =
		Cvar_RegisterString("sv_wwwBaseURL", "", 1, "The base url to files for downloading from the HTTP-Server");
	sv_wwwDlDisconnected = Cvar_RegisterBool("sv_wwwDlDisconnected", qfalse, 1,
	...
	sv_padPackets = Cvar_RegisterInt("sv_padPackets", 0, 0, 0x7fffffff, 0,
	...
	sv_mapRotationCurrent =
		Cvar_RegisterString("sv_mapRotationCurrent", "", 0, "List of remaining maps server has to play");
	sv_nextmap = Cvar_RegisterString("nextmap", "", 0,
		"String to execute on map_rotate. This will override the default maprotation of CoD4 with the Quake3 style "
		"maprotation");
	sv_randomMapRotation =
		Cvar_RegisterBool("sv_randomMapRotation", qfalse, CVAR_ARCHIVE, "Mixes up maps in rotation when true");

	sv_timeout = Cvar_RegisterInt("sv_timeout", 40, 0, 1800, 0,
		"Seconds to keep a client on server without a new clientmessage");
	sv_connectTimeout = Cvar_RegisterInt("sv_connectTimeout", 90, 0, 1800, 0,
	...
	sv_modStats = Cvar_RegisterBool("ModStats", qtrue, 0,
		"Flag whether to use stats of mod (when running a mod) or to use stats of the Cod4 coregame");
	sv_authorizemode = Cvar_RegisterInt("sv_authorizemode", 1, -1, 1, CVAR_ARCHIVE,
		"How to authorize clients, 0=acceptall(No GUIDs) 1=accept no one with invalid GUID");
	sv_showasranked = Cvar_RegisterBool("sv_showasranked", qfalse, 0,
		"List the server in serverlist of ranked servers even when it is modded");
	sv_statusfile = Cvar_RegisterString("sv_statusfilename", "serverstatus.xml", CVAR_ARCHIVE,
		"Filename to write serverstatus to disk");
	g_mapstarttime =
		Cvar_RegisterString("g_mapStartTime", "", CVAR_SERVERINFO | CVAR_ROM, "Time when current map has started");
	g_friendlyPlayerCanBlock =
		Cvar_RegisterBool("g_friendlyPlayerCanBlock", qfalse, 0, "Flag whether friendly players can block each other");
	g_FFAPlayerCanBlock = Cvar_RegisterBool("g_FFAPlayerCanBlock", qtrue, 0,
	...
	sv_uptime =
		Cvar_RegisterString("uptime", "", CVAR_SERVERINFO | CVAR_ROM, "Time the server is running since last restart");
	sv_autodemorecord =
		Cvar_RegisterBool("sv_autodemorecord", qfalse, 0, "Automatically start from each connected client a demo.");
	sv_demoCompletedCmd = Cvar_RegisterString("sv_demoCompletedCmd", "", com_securemode ? CVAR_INIT : 0,
		"This program will be executed when a demo has been completed. You have to specify an executable file located "
		"in fs_homepath/apps/. The arrived filename will be passed as argument.");
	sv_screenshotArrivedCmd = Cvar_RegisterString("sv_screenshotArrivedCmd", "", com_securemode ? CVAR_INIT : 0,
		"This program will be executed when a new screenshot has arrived. You have to specify an executable file "
		"located in fs_homepath/apps/. The arrived filename will be passed as argument.");
	sv_mapDownloadCompletedCmd = Cvar_RegisterString("sv_mapDownloadCompletedCmd", "", com_securemode ? CVAR_INIT : 0,
		"This program will be executed when a downloaded map was received. You have to specify an executable file "
		"located in fs_homepath/apps/. The arrived usermaps/mapname will be passed as argument.");
	sv_consayname = Cvar_RegisterString("sv_consayname", "^2Server: ^7", CVAR_ARCHIVE,
		"If the server broadcast text-messages this name will be used");
	sv_contellname = Cvar_RegisterString("sv_contellname", "^5Server^7->^5PM: ^7", CVAR_ARCHIVE,
		"If the server broadcast text-messages this name will be used");
	...
	sv_maxclients = Cvar_RegisterInt("sv_maxclients", 16, 1, 64, CVAR_INIT | CVAR_SERVERINFO,
		"Maximum number of clients that can connect to a server");
	sv_clientSideBullets = Cvar_RegisterBool("sv_clientSideBullets", qtrue, 8,
	...
	sv_floodProtect = Cvar_RegisterInt("sv_floodprotect", 4, 0, 100000, 5,
		"Prevent malicious lagging by flooding the server with commands. Is the number of client commands allowed to "
		"process");
	...
	sv_referencedIwdNames =
		Cvar_RegisterString("sv_referencedIwdNames", "", 0x48, "Names of all referenced IWD files used by the server");
	...
	sv_referencedFFCheckSums =
		Cvar_RegisterString("sv_referencedFFCheckSums", "", 0x48, "Checksums of all referenced FastFiles");
	sv_referencedFFNames =
		Cvar_RegisterString("sv_referencedFFNames", "", 0x48, "Names of all referenced FastFiles used by the server");
	sv_serverid = Cvar_RegisterInt("sv_serverid", 0, 0x80000000, 0x7fffffff, 0x48,
		"The current id of server to let client and server distinguish between gamestates");
	...
	sv_showAverageBPS =
		Cvar_RegisterBool("sv_showAverageBPS", qfalse, 0, "Show average bytes per second for net debugging");
	sv_botsPressAttackBtn =
		Cvar_RegisterBool("sv_botsPressAttackBtn", qtrue, 0, "Allow testclients to press attack button");
	sv_debugRate = Cvar_RegisterBool("sv_debugRate", qfalse, 0, "Enable snapshot rate debugging info");
	sv_debugReliableCmds =
		Cvar_RegisterBool("sv_debugReliableCmds", qfalse, 0, "Enable debugging information for reliable commands");
	...
	sv_authtoken = Cvar_RegisterString("sv_authtoken", "", 0,
		"Token to register on masterserver. You can get it from http://cod4master.cod4x.ovh");
...
void SV_LoadLevel(const char *levelname)
{
	...
	//"localized_british_iw00 localized_korean_iw00 localized_taiwanese_iw00 localized_japanese_iw00
	// localized_chinese_iw00 localized_thai_iw00 localized_leet_iw00 localized_czech_iw00 localized_english_iw0
	// localized_french_iw0 localized_german_iw0 localized_italian_iw0 localized_spanish_iw0 localized_polish_iw0
	// localized_russian_iw0 localized_chinese_iw0");
...
void SV_SetConfigValueForKey(int start, int max, const char *key, const char *value)
{
	...
	if (i == max)
	{
		...
		for (i = 0; i < max; ++i)
		{
			Com_Printf(CON_CHANNEL_SERVER, "%i: %i ( %s )\n", i + start, SV_GetConfigstringIndex(start + i),
				SL_ConvertToString(SV_GetConfigstringIndex(start + i)));
		}
...
void SV_BotUserMove(client_t *client)
{
	...
	if (level.clients[num].sess.archiveTime == 0)
	{
		...
		if (g_botai[num].doMove)
		{
			...
			// Com_Printf(CON_CHANNEL_SERVER,"val: (%3d, %3d), distance: %f ", ucmd.forwardmove, ucmd.rightmove,
			// distance); Com_Printf(CON_CHANNEL_SERVER,"speed: (%d, %d)", ucmd.forwardmove, ucmd.leftmove);
			// Com_Printf(CON_CHANNEL_SERVER,"origin: (%3.3f, %3.3f, %3.3f)", ent->r.currentOrigin[0],
			// ent->r.currentOrigin[1], ent->r.currentOrigin[2]); Com_Printf(CON_CHANNEL_SERVER,"\n");
...
__optimize3 __regparm1 qboolean SV_Frame(unsigned int usec)
{
	...
	*/
	...
	/*
		for(i = 0; i < sv_maxclients->integer; ++i)
		{
			...
			{
				Com_Printf(CON_CHANNEL_SERVER,"^1Debug Spawn angles changed: ^7ent->r.currentAngles changed new: %.2f,
	   %.2f, %.2f\n", svs.clients[i].gentity->r.currentAngles[0], svs.clients[i].gentity->r.currentAngles[1],
			...
			{
				Com_Printf(CON_CHANNEL_SERVER,"^1Debug Spawn angles changed: ^7ent->client->ps.viewangles changed new:
	...
	*/
	...
	if (svs.time > svs.frameNextSecond)
	{ // This runs each second
		...
		if (svs.time > svs.frameNextTenSeconds)
		{ // This runs each 10 seconds
...
//  Cvar_SetString(nextmap, "map_restart");
	...
	Com_Printf(CON_CHANNEL_DONT_FILTER,
		"By using this software you agree to the usage conditions you can find at "
		"https://github.com/callofduty4x/CoD4x_Server#usage-conditions-for-server-hosters\n");
	Com_Printf(CON_CHANNEL_SERVER, "-----------------------------------\n");
